...
const ALLOWED_MODIFICATIONS = new Set<string>([
  ...
  ".opencode/tui.json", // select the Volt brand theme for the dev-time TUI (configured here, not in opencode.jsonc)
])
// The former GUI/binary seams (packages/{ui,app,desktop}, packages/opencode/src) are GONE: the product ships
// volt-desktop over STOCK, user-provided opencode (made Volt-aware only by OPENCODE_CONFIG_DIR), so nothing of
// opencode's GUI or binary is forked anymore. See openspec/changes/minimize-opencode-fork. Editing any of those
// upstream files is now a divergence again — the point of the de-fork.
...
function selfTest(): void {
  const cases: { name: string; lines: string[]; allowed: number; violations: number }[] = [
    ...
    { name: "de-forked: logo / app-name GUI edits are violations again", lines: ["M\tpackages/ui/src/components/logo.tsx", "M\tpackages/desktop/src/main/index.ts", "M\tpackages/desktop/electron-builder.config.ts"], allowed: 0, violations: 3 },
    { name: "de-forked: GUI session-panel edit is a violation again", lines: ["M\tpackages/app/src/pages/session.tsx", "M\tpackages/app/package.json"], allowed: 0, violations: 2 },
    { name: "de-forked: desktop IPC edits are violations again", lines: ["M\tpackages/desktop/src/preload/index.ts", "M\tpackages/desktop/electron.vite.config.ts"], allowed: 0, violations: 2 },
    { name: "de-forked: opencode/src binary edits (updater / tui worker) are violations again", lines: ["M\tpackages/opencode/src/installation/index.ts", "M\tpackages/opencode/src/cli/cmd/tui.ts"], allowed: 0, violations: 2 },
    { name: "an unrelated packages/app edit is a violation", lines: ["M\tpackages/app/src/app.tsx"], allowed: 0, violations: 1 },
    ...
    { name: "mixed real-world set", lines: ["M\tbun.lock", "A\tvolt-scripts/x.ts", "M\tpackages/volt-lsp-iec/src/y.ts", "M\tpackages/server/src/z.ts"], allowed: 1, violations: 1 },
