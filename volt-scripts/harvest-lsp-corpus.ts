...
// `verbose` also returns referenced-library element SIGNATURES (build-free) as ordinary `changed` items under
// each library's Library Manager folder — the harvest wants the complete picture for the LSP corpus. Objects
// with no compiler ground truth (excluded-from-build, dead/uncompiled) are omitted by the bridge, so the corpus
// contains only analyzable files (no in-file markers).
...
console.log(`wrote ${written} files (${kind} KIND source incl. library signatures, ${lib} .library, ${dev} .device, ${info} .projectinfo, ${misc} .trace/.recipe/.symbols/.task), skipped ${skipped}`)
