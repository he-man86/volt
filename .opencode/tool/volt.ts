...
// This is the repo's OWN dev tool. Its twin for USER projects is generated by volt-git's
// `opencode-config.ts` → `toolSource()` at `volt init` (same behaviour; VOLT_BIN is the bundled exe there,
// this repo build here). They can't share a module — the generated one must be self-contained — so keep
// the two in sync by hand. This file lives at <repoRoot>/.opencode/tool/volt.ts, so the built CLI is a
// fixed two levels up — resolved from the file (not ctx.directory) so it works in any open project.
