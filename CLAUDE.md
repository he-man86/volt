...
- **`volt-desktop`** (`@volt/desktop`) — the standalone desktop app: an Electron window whose whole content is the IDE panel over `volt-control` (connection / sync / diagnostics). It embeds no editor and no agent, opens from its own exe (the Start Menu shortcut the connector writes targets it — the tray itself has no launcher), and remembers the last workspace it was bound to (`recent.ts` — without that memory a returning user is offered a brand-new workspace every launch).
...
**Volt operates no backend.** It sells a €19/month subscription for the toolchain through [Polar](https://polar.sh), which is the **merchant of record** — payment, EU VAT, licence-key issuance, revocation on cancellation and the customer portal are all theirs. There is no database, no auth system, no dashboard and no Stripe integration. The CLI holds a licence key, caches a verdict, and enforces a free allowance of 3 bound projects locally; the connector keeps that cache warm but is never required. The plan for this was **closed unfinished on 2026-09-03** (`openspec/changes/archive/2026-09-03-sell-cli-subscription`, with a close-out): no licensing code was ever written, so the CLI enforces nothing today, and the storefront gates an unset checkout URL behind `COMING_SOON`. Treat the paragraph above as the intent that was being explored, not as shipped behaviour.
...
- **`packages/volt-cli`** (`@volt/cli`) — the whole toolchain in one C# solution over Windows **named pipes** (no HTTP):
  - **the bridges** — one live IDE exposed over the pipe. **`Volt.Engine` holds everything shareable; only irreducible vendor glue lives in an IDE host (`Volt.Ide.Codesys` / `Volt.Ide.Twincat`, each = driver + pipe host).** The parity boundary is the pipe wire (not the driver), so both vendors serve byte-identical responses for the same project. See `packages/volt-cli/ARCHITECTURE.md` — read it before touching bridge code; it documents the engine layer stack (`Item/` → `Format/` → `Ide/` contract → `Sync/`), the rule that **`ICodeStore` speaks `ItemContent` and nothing above the vendor seam knows a vendor's representation**, and the **load-bearing CODESYS↔Beckhoff asymmetries that must not be "unified"** (censused in `packages/volt-cli/src/Volt.Engine/Ide/DIALECT.md`). **PLCopen is deleted** — content no longer travels as a document; each driver reaches its vendor's own form below the seam, and the two vendors ship an identical `NWLObject` model that differs only in ACCESS (DIALECT N1).
...
Editable graphical bodies (FBD/LD) round-trip the vendor's own model ⇄ **network text**; CFC, SFC and IL are **unsupported** — they materialize as a marker and are refused on push, never flattened to ST. The format is specified in `packages/volt-cli/docs/network-text.html`; wires use inline `LET`. FBD and LD share one model (`Format/Network/NetworkModel`), one text format and one pipeline — a ladder's contacts and coils lower into the same node tree an FBD network uses — so they are never split in the code. (It was briefly called "VG"; that alias is retired.) `packages/volt-cli/docs/items.html` documents the vendor-neutral item-type table (`Volt.Engine/Item/ItemKind` is the source of truth). **`packages/volt-cli/docs/` is a generated-and-gated static doc site** — open `docs/index.html`; the fact tables come from the code via `test/Volt.Engine.Tests/docs/DocDataTests.cs` and the build fails if they drift (`VOLT_WRITE_DOCS=1` to regenerate).
...
- **Source of truth for invariants is the code + each package's `README.md`/`ARCHITECTURE.md`** (e.g. `volt-cli/ARCHITECTURE.md`, `volt-lsp-iec/docs/`), not a parallel spec tree. **OpenSpec is `openspec/changes/` only** — in-flight proposals + the decision log (`openspec list`); the archived `specs/` capability tree was removed (it drifted) and its load-bearing invariants folded into the package docs. **`openspec archive` RE-CREATES `openspec/specs/`** from the change's spec delta — delete it after archiving; the change's own `specs/*.md` travels into `changes/archive/<date>-<name>/` and that is the record. Check that delta still describes what was actually BUILT before archiving: a delta written at proposal time can assert a design the work went on to measure and reject, and archiving files it as a requirement.
