...
- [ ] `Wire/RefsFetch.cs` — remove the `OmitDeadCode` property (JsonPropertyName `omitDeadCode`) from `FetchRequest`.
- [ ] `Sync/FetchService.cs` — remove the dead-code block (the `request.OmitDeadCode && GetCompiledPouNames()` removal
...
- [ ] `Ide/IIdeSession.cs` + `Ide/DriverBase.cs` — remove `GetCompiledPouNames` (declaration + abstract member).
- [ ] `Codesys/Driver/CodesysDriver.cs` + `Codesys/Ide/CodesysObjectModel.cs` — remove the `GetCompiledPouNames`
      override + its compiled-model reflection. (The compile context stays — `ExtractLibrarySignatures` still uses it.)
- [ ] `Beckhoff/Driver/BeckhoffDriver.cs` — remove the `GetCompiledPouNames` override (returned null).
- [ ] Confirm no other consumer of `GetCompiledPouNames` / `OmitDeadCode` remains (grep both, incl. openapi.yaml).

## 2. Bridge — tests
- [ ] `test/Volt.Bridge.Tests/FakeIde.cs` — remove `CompiledPous` + `GetCompiledPouNames`.
- [ ] `test/Volt.Bridge.Tests/FetchExclusionTests.cs` — remove the `omitDeadCode` dead-code test; KEEP the
      exclude-from-build tests (excluded item omitted, siblings fold beside it, `(unresolved)` surfacing).
- [ ] Full C# unit suite green.

## 3. Harvest + corpus
- [ ] `volt-scripts/harvest-lsp-corpus.ts` — drop `omitDeadCode: true` (and its comment); harvest returns all code.
- [ ] Re-harvest the corpus (now includes dead code). NOTE the pre-existing Windows long-path issue on
      `packages/volt-lsp-iec-next/test-corpus/` — harvest to the legacy `volt-lsp-iec/test-corpus` path the
      `-next` corpus test already references, or resolve the long-path first.

## 4. LSP (`volt-lsp-iec-next`) — structural dead-code detection
- [ ] `src/analysis/config.ts` — add `diagnoseDeadCode: boolean` (default `false`). Place it on the analysis
      config (NOT the style-`lints` bag — it changes WHAT is analyzed, not adds a lint). Plumb it through
      `resolveConfig`/`ResolvedConfig`.
- [ ] Reachability pass (analysis layer, project-level — NOT a per-document `CHECKS` entry). Inputs it needs that
...
- [ ] Wire the dead set + `diagnoseDeadCode` into diagnostics: when `diagnoseDeadCode` is false, SUPPRESS all
...
- [ ] (If a user-facing toggle is wanted) plumb `initializationOptions.diagnoseDeadCode` in the server — today
      `resolveConfig` is only ever called with `{ vendor }`. Otherwise the flag is code-default only for now.

## 5. LSP — tests
- [ ] Unit: a PROGRAM→FB call chain keeps the FB live; an FB never called/instantiated is dead; an FB reachable
...
- [ ] Corpus 0-FP gate holds over the re-harvested corpus (dead code present, suppressed by default).

## 6. Specs
- [ ] `bridge-protocol`: replace "The bridge returns only items with compiler ground truth" — excluded omitted
      (keep), dead code NO LONGER omitted (returned as ordinary source). (delta in this change)
- [ ] `st-language-server`: REMOVE "Diagnostics skip build-excluded objects" (excluded objects are never delivered
...
- [ ] Reconcile the dangling cross-references to the retired marker / `excludeFromBuild` wire flag in both specs
