...
### Requirement: The merge conflict surface is the 13 enumerated seams

The entire divergence from upstream SHALL be exactly 13 modified upstream files, in four
...
GUI panel (3: `packages/app/package.json`, `helpers.ts`, `session-side-panel.tsx`), and desktop
IPC (4: `packages/desktop/{package.json, electron.vite.config.ts, src/main/index.ts, src/preload/index.ts}`).
`check-divergence` SHALL pass only when no upstream file outside this set is modified or deleted.

#### Scenario: A clean fork enumerates exactly the 13 seams
- **WHEN** `check-divergence` runs against `upstream/dev` at `HEAD`
- **THEN** it lists the 13 seams and reports clean
