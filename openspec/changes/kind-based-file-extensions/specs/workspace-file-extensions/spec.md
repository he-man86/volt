...
**Reason**: Superseded — the extension now names the item's KIND so a file's purpose is visible at a glance, replacing the single `.st`.
**Migration**: See ADDED "Writable source items are named by kind"; on first pull, `*.st` files re-materialize under their kind extension.

### Requirement: Read-only kinds keep distinct extensions
**Reason**: Superseded — read-only POUs (CFC/SFC) are now kind-named too, so read-only no longer rides on the extension; only opaque reference kinds keep their own extension.
**Migration**: See ADDED "Read-only is a per-item flag, not the extension".

### Requirement: Access is derived from the extension, kind from content
**Reason**: Superseded — a POU's access can no longer be read from its (kind-based) extension; it comes from a per-item wire flag. Kind is still recovered from content.
**Migration**: See ADDED "Access comes from a per-item flag; kind from content".

### Requirement: The collapse changes wire identity once
**Reason**: Superseded — the scheme change is a re-materialization to kind extensions, not a collapse to `.st`.
**Migration**: See ADDED "The scheme change re-materializes once".
