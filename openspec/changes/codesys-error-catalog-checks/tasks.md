...
- [x] 2.4 Rewrote the stance section of `13-error-messages.md` (2026-07-10): catalog is the coverage checklist; our diagnostics map to `Cnnnn` as metadata (own `source`/`code` unchanged). Superseded the old "we do not own these codes" position.
...
- [x] 6.3 Coverage source of truth is now honest and machine-backed (2026-07-10): every open code carries a `triage` tag + `note` in `error-catalog.json` (parser/pragma/resolution/optionGated/ideOnly/skip), 35 codes reclassified `checkable→ide-only`, and TRIAGE.md refreshed from the catalog (was stale at implemented:10). "What's open and why" is now derivable from the catalog, not prose that rots.
