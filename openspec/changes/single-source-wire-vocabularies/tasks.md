...
- [ ] 0.1 Each phase is its own commit. Introduce the constant initialized to the EXACT current literal, then swap
      call sites literal→reference in the SAME commit, so the diff is provably value-preserving.
- [ ] 0.2 After every phase: `dotnet test test/Volt.Engine.Tests/` (incl. `WireContractParityTests`) +
...
- [ ] 1.1 In `Volt.Engine/Workspace/ItemKind.cs`: add a nested `static class Kinds` of `const string` members for
...
- [ ] 1.2 NOTE (corrected after reading the code): `PushService.PouKindToCode`/`ChildKindToCode` (530-547) are NOT a
...
- [ ] 1.3 Replace the raw kind literals with the consts across the ~15 sites the audit listed: `PushService.cs`
...
- [ ] 1.4 Confirm `WireContractParityTests` + `check-wiring.ts` still green (kind values are a cross-language contract).

## 2. Op codes + progress/active-op labels — one `Ops` const class

- [ ] 2.1 Add `Ops` (`static class`, `const string`) in `Volt.Engine/Wire` with the 9 op names, values identical to
      today's wire strings.
- [ ] 2.2 Reference it in `BridgePipeHost.cs` (the dispatch `case`s + `AllowedWhilePaused` + the `Busy(...)` labels),
      `BridgeClient.cs` (the `.Call(...)` sites), and both connector sources (`CodesysProjectSource.cs`,
      `PipeProjectSource.cs`) + `HealthProbe.cs:39`.
- [ ] 2.3 Route the `ProgressFrame.Operation` labels (`RefsService`, `FetchService`, `PushService`, `BuildService`,
      `ProjectSnapshot`, `Commands.cs`) through the SAME `Ops` consts — the vocabulary that was duplicated a second time.
- [ ] 2.4 Fix the `HealthProbe.cs:25` doc that says the active op is `"pull"` — the wire carries `"fetch"`. (Doc-only.)

## 3. Vendor id + display name — `Vendors` (C#) and one shared TS source

- [ ] 3.1 Add `Vendors` (`const string` id + display name) in `Volt.Cli.Transport` beside `PipeNames`; `ForVendor`
      keys off the id. Values identical to today's (`codesys`/`CODESYS`, `twincat`/`TwinCAT`).
- [ ] 3.2 Reference it across the connector/CLI/driver layers the audit listed (`Program.cs:52`, `BridgeResolver.cs:20`,
...
- [ ] 3.3 TS: pick the single source (`volt-control/src/bridge/health.ts` already has `Vendor`/`VENDORS`; `display.ts`
...
- [ ] 4.1 Add `HealthStatus` consts (`Healthy`/`Degraded`/`Unavailable`) — on `HealthResponse` or beside it in
...
- [ ] 5.1 Replace the 3 raw `"PLC_DISCONNECTED"` literals (`BridgeClient.cs:68`, `BridgeResolver.cs:33`, `:44`) with the
...
- [ ] 5.2 Name `"INTERNAL_ERROR"` (`PipeServer.cs:99`) as a `BridgeErrorCodes` member and reference it.

## 6. Cross-language: only the reference-extension gap

- [ ] 6.1 Extend `volt-scripts/check-wiring.ts` to also cross-check the reference extensions (`.library`/`.device`/
...
- [ ] 7.1 Add a C# test on the `VendorParityGuardTests` model: strip comments, fail if a raw literal of a centralized
