...
We **do not own these codes**. The LSP's own diagnostics (`source: "volt-lsp-codesys"`) use our own message text. We don't try to issue CODESYS codes from the LSP — that would be misleading, because:
