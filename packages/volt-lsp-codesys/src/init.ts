/**
 * `volt-lsp-codesys init` — copy the language reference corpus into a
 ...
 *     ├─ .volt-lsp-codesys-version           ← version marker
...
const VERSION_MARKER_REL_PATH = `${SKILL_DIR_REL}/.volt-lsp-codesys-version`;
...
metadata:
  ...
  vendor: twincat
  source-package: "@opencode-ai/volt-lsp-codesys"
  installed-by: "volt-lsp-codesys init"
---

## Purpose

TwinCAT 3 Structured Text language reference, installed into this workspace by \`volt-lsp-codesys init\` (run via \`volt init\`). Use when writing or reviewing \`.st\` files.
...
Authoritative CODESYS Structured Text language reference, installed into this workspace by \`volt-lsp-codesys init\` (run via \`volt init\`). Use when writing or reviewing \`.st\` files.
	...
	try {
		...
		throw new Error(
			`Source corpus not found at ${sourceCodesysDir}. ` +
				`The volt-lsp-codesys package may be missing the docs/ tree.`,
		);
	}
