...
function main(argv: readonly string[]): number {
	...
	if (cmd === "--version" || cmd === "-v") {
		console.log(`volt-lsp-codesys ${VERSION}`);
		return 0;
	}
	...
	if (cmd === "lex") {
		const file = rest[0];
		if (file === undefined) {
			console.error("usage: volt-lsp-codesys lex <file>");
			return 1;
		}
...
Usage:
  volt-lsp-codesys lex <file>     Print token stream for a .st file
  volt-lsp-codesys --stdio        Run as LSP over stdio (LSP 3.17)
  volt-lsp-codesys --version      Print version
  volt-lsp-codesys --help         Show this help`);
}
