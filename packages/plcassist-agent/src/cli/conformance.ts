 ...
 *
 * Requires:
 *   - Bridge on 127.0.0.1:8555 (PLCASSIST_BRIDGE_PORT)
...
function plc(workspace: string, ...args: string[]): CliResult {
	const r = spawnSync("node", [CLI_PATH, ...args, "--workspace", workspace, "--port", String(BRIDGE_PORT)], {
		encoding: "utf-8",
		env: { ...process.env, PLCASSIST_BRIDGE_PORT: String(BRIDGE_PORT) },
	});
...
async function main(): Promise<void> {
	...
	if (!existsSync(CLI_PATH)) {
		console.error(`pre-flight: CLI not built. Run \`pnpm -C packages/plcassist-agent build\` first.`);
		process.exit(1);
	}
...
async function runAllScenarios(ws: string): Promise<void> {
	...
	section("S01 — `plc init` binds workspace to the IDE project");
	{
		...
		assert(existsSync(join(ws, ".plcassist", "config.json")), ".plcassist/config.json exists");
		assert(existsSync(join(ws, ".plcassist", "snapshot")), ".plcassist/snapshot/ exists");
		const cfg = JSON.parse(readFileSync(join(ws, ".plcassist", "config.json"), "utf-8"));
