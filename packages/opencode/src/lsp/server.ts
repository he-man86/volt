...
/**
 * Volt LSP for IEC 61131-3 PLC languages — Structured Text (`.st`)
 * plus the graphical body containers (`.fbd` / `.ld` / `.sfc` /
 * `.cfc`) and related declaration files (`.gvl` / `.dut` / `.itf`).
 *
 * The server resolves via the workspace package
 * `@opencode-ai/volt-lsp` (path inside the volt monorepo:
 * `packages/volt-lsp-st`). It's an in-workspace dep, so Module.resolve
 * finds it when this opencode build is run inside the volt repo;
 * outside the repo the resolve silently fails and the server is
 * skipped (same fall-through behaviour as Pyright / TypeScript when
 * the required dep isn't installed).
 *
 * Root: project root is the workspace directory — there's no
 * sentinel file we look for. The LSP keys cross-file resolution off
 * the workspace tree it receives via `initialize`.
 */
export const VoltLspSt: Info = {
  id: "volt-lsp-st",
  extensions: [".st", ".gvl", ".dut", ".itf", ".fbd", ".ld", ".sfc", ".cfc"],
  root: async (_file, ctx) => ctx.directory,
  async spawn(root, ctx) {
    // 1. Global PATH binary (installed via npm install -g @opencode-ai/volt-lsp)
    const fromPath = which("volt-lsp-st")
    if (fromPath) {
      return {
        process: spawn(fromPath, ["--stdio"], { cwd: root }),
      }
    }
    // 2. Workspace package (running opencode from inside the volt monorepo)
    const bin = Module.resolve("@opencode-ai/volt-lsp/dist/bin.js", ctx.directory)
    if (bin) {
      return {
        process: spawn("node", [bin, "--stdio"], { cwd: root }),
      }
    }
    log.info("volt-lsp-st not found; install @opencode-ai/volt-lsp globally to enable PLC language support")
  },
}
