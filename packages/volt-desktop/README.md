...
- **IDE panel** — the rail + panel render over `@opencode-ai/volt-control` (the same UI-agnostic core `volt-vscode` uses — *share the logic, not the pixels*): **IDE Sync** (drift + pull/push/build), **Diagnostics** (headless LSP collector), **Bridge** (health/port). Bridge lifecycle is the connector's job, never this shell.
...
| `preload.cjs` | exposes `window.volt` (the `@opencode-ai/volt-control` IPC contract) to the shell renderer |
