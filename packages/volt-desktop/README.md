...
- **Active workspace follows opencode's ROUTE** — opencode's GUI names the project in its URL: project pages are `/<base64url(project directory)>/…`, home is `/`. Read live from the running app (`/QzpcVXNlcnNcbWFyY2VcRG9jdW1lbnRz…/session/ses_060a…` decodes to a real workspace path). So a navigation — and only a navigation — moves the binding: a project page binds (or rebinds) it, `/` releases it, and an unrecognised route classifies as *tells us nothing* and HOLDS the current binding (opencode's `/new-session` takes that path). `VOLT_WORKSPACE` is a dev override. The decision is the pure reducer in `binding.ts`; every navigation, bind and release is logged to `%LOCALAPPDATA%\Volt\logs\desktop-<date>.log`, and the whole chain is driven live over CDP by `test/e2e/opencode-binding.test.ts`.
  - This REPLACED a sniff of opencode's HTTP request stream (`x-opencode-directory` / `?directory=`), which was wrong in **both** directions: on home its client keeps naming the LAST project (it is constructed with a directory and stamps it on every request), so Volt bound — and auto-connected — a project the user never opened; and a project with no session yet emits nothing but `/global/health`, so opening a project bound nothing until you sent a chat message. See `openspec/changes/archive/2026-07-29-desktop-connection-flow/observations.md`.
  - **The bridge connection follows the active project view** (over the `connector-session-model`) — binding a project **declares an interest** in it (`enterWorkspace`), releasing it (home route) **drops** the interest (`leaveWorkspace`), and app quit `shutdownSession()`s the whole session (via `before-quit`, bounded ~1.5s); the same shared `@volt/control` lifecycle VS Code drives from `activate`/`deactivate`. The connector serves a project iff ≥1 live session wants it, so **one project open in two UIs no longer disconnects each other** (the union replaced the deferred refcount — see `openspec/changes/connector-session-model/`). Manual Connect/Disconnect stay as an override.
