...
- **Active workspace follows opencode (STICKY)** — opencode's server has **no queryable "current project"** and is **session-scoped**: until you start a chat, opencode reports its `global` scope for both its home screen *and* every new-session draft, and it only reveals a plaintext project directory once a session exists (the `?directory=` on GET requests is encrypted). All verified live — see `openspec/changes/desktop-connection-flow/observations.md`. So the shell sniffs the GUI's requests and **binds to a real project directory when opencode first reveals one (on the first chat), rebinding when you work in a different project — and never releases**. It does NOT release on `global`, because opencode can't tell its home screen from a new-session draft, so releasing would unbind the panel every time you open a draft. A `global` signal only clears the cold-start "Connecting…" so the create-a-workspace surface can show. `VOLT_WORKSPACE` is a dev override; `VOLT_BIND_DEBUG=1` logs every request's `dir → classification`. The bind decision is the pure reducer in `binding.ts`.
