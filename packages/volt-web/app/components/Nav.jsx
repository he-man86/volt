    ...
    <div className="nav-item">
      ...
      <div className="nav-menu">
        <div className="nav-menu-panel">
          {item.items.map((sub) => (
            <Link key={sub.href} to={sub.href} className="nav-menu-item">
              {sub.label}
            </Link>
          ))}
        </div>
...
// Client-side navigation no longer unmounts this panel on its own, so the click handler closes it.
      ...
      {NAV.map((item) =>
        item.items ? (
          <div key={item.label} className="nav-mobile-group">
            ...
            {item.items.map((sub) => (
              <Link key={sub.href} to={sub.href} className="nav-mobile-link">
                {sub.label}
              </Link>
            ))}
          </div>
        ) : (
          <Link key={item.label} to={item.href} className="nav-mobile-link nav-mobile-top">
            {item.label}
          </Link>
        ),
      )}
