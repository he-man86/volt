...
// The deploy bakes VITE_CONSOLE_URL to the console's real host (infra/www.ts → `https://app.${domain}`). This
// default is the fallback for a local `vite dev`: it points at the PRODUCTION console (app.volt-ai.dev) so a
// forgotten env is still safe; target a dev console explicitly with `VITE_CONSOLE_URL=https://app.dev.volt-ai.dev`.
