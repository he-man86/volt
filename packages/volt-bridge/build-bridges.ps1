...
# The bridges share Volt.Bridge.Core (HTTP server, handlers, ST/PLCopen logic, openapi).
param(
...
Write-Output "`n[Test] Volt.Bridge.Tests"
& $DOTNET test "$ROOT\test\Volt.Bridge.Tests\Volt.Bridge.Tests.csproj" -c Release --nologo -v q
...
Write-Output "`n[2/3] Volt.Bridge.Codesys"
& $DOTNET build "$ROOT\src\Volt.Bridge.Codesys\Volt.Bridge.Codesys.csproj" -c Release -o "$DIST\Codesys" --nologo -v q
if ($LASTEXITCODE -ne 0) { Write-Output "  FAILED"; exit 1 }
...
if ($ApiTests) {
    ...
    Get-Process -Name Volt.Bridge.Beckhoff -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 1
    Write-Output "  launching dist\Beckhoff\Volt.Bridge.Beckhoff.exe"
    $bridgeOut = Join-Path $env:TEMP "volt-apitest-bridge.out.log"
    $bridgeErr = Join-Path $env:TEMP "volt-apitest-bridge.err.log"
    $owned = Start-Process "$DIST\Beckhoff\Volt.Bridge.Beckhoff.exe" -PassThru -WindowStyle Hidden -RedirectStandardOutput $bridgeOut -RedirectStandardError $bridgeErr
...
Write-Output "   dist\Beckhoff\Volt.Bridge.Beckhoff.exe   (worker, also bundled in Connector)"
