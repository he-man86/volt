...
NETWORK <n> <FBD|LD>
  VAR_TEMP
    <name> : <type>;        -- only NAMED wires (fan-out results g*, en*, opaque leaves i*); types writer-owned
  END_VAR
  <statements>
END_NETWORK
```
...
- **Named result** (`g := (…)` / `g := FN(…)`): a block result that **fans out** (2+ consumers) is named (else it's
  inlined). The name lets both consumers share the one box.
- **Opaque leaf** (`i := <text>`): an inVariable whose text has whitespace or parens (`a + 1`, `NOT x`) can't sit at
  an operand position as one token, so it's named and gets its own statement. Its text may NOT alias a temp.
...
- **EN/ENO** box: `en := <src>; IF en THEN result := <expr>; END_IF` — the IF is its faithful ST. `en` is the box's
  enable echo (its `ENO`), so a downstream box chains off it (`en2 := NOT en1`).
- **Modifiers**: `NOT` (negation), `RISING`/`FALLING` (edge), `SET`/`RESET` (coil storage) ride on the CONSUMER
  (the operand/sink), never as their own statement (`out := NOT g1`, not `g2 := NOT g1`).
...
| `VG_LEAF_REFERENCES_TEMP` | An opaque leaf's text aliases a temp (`g2 := NOT g1`) — a NOT/edge rides on the consumer, and an expression over temps is written inline (fully parenthesised) at its consumer. |
