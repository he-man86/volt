...
namespace Volt.Bridge.Core.Graphical
{
    ...
    public static class PlcOpenDocument
    {
        ...
        /// or can safely discard because they are cosmetic presentation metadata regenerated by the IDE
        /// on import. <c>vendorElement</c> is editor-specific rendering info. <c>leftPowerRail</c>,
        /// <c>rightPowerRail</c>, <c>contact</c>, and <c>coil</c> are LD ladder-diagram visuals that
        /// CODESYS regenerates (TwinCAT wraps LD in an <c>&lt;FBD&gt;</c> body and stores the ladder
        /// view in separate metadata, so they never appear). Adding a genuinely structural element here
        /// without VG support would silently drop functional logic — every entry must be affirmatively
        /// confirmed as cosmetic.</summary>
