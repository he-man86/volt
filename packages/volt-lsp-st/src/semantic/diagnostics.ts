...
/**
 * Detect binary operator type mismatches in simple statements of
 * shape `<lhs> := <id> <op> <id> ;`. Only catches the two specific
 * gaps the conformance harness flagged:
 *   - `MOD` with non-integer operand (TC: "'MOD' is not defined for 'REAL'")
 *   - arithmetic (+, -, *, /) mixing BOOL with numeric (TC:
 ...
 * method the interface declares. Mirrors TC's "method not
 * implemented" error.
 *
 * Resolves each interface name to a scope (cross-file via the project
 * tree). For each interface method, checks the FB's own scope has a
 * child scope with the matching method name. Signature compatibility
 * is NOT checked here — pure presence-or-absence. Misnamed overrides
 * surface as separate issues; this catches the common "forgot to
 * implement" mistake.
 */
function checkInterfaceImplementations(
	parseResult: ParseResult,
	project: Scope,
	out: DiagnosticItem[],
): void {
	for (const unit of parseResult.units) {
		...
		// Names of all method-shaped children on this FB.
		...
		for (const ifaceName of implementsList) {
			...
			// Walk the interface's own symbols (interface methods +
			// properties are flat children of the interface scope).
...
function findScopeByName(project: Scope, name: string): Scope | undefined {
