...
describe("parser: FUNCTION", () => {
	it("FUNCTION with simple return type", () => {
		...
		`);
		expect(errors).toEqual([]);
		const f = units[0] as import("./ast.js").Function;
...
describe("parser: ACTION", () => {
	it("simple ACTION", () => {
		...
		`);
		expect(errors).toEqual([]);
		const a = units[0] as import("./ast.js").Action;
...
describe("parser: INTERFACE", () => {
	it("interface with method signatures", () => {
		...
		`);
		expect(errors).toEqual([]);
		const iface = units[0] as import("./ast.js").Interface;
...
describe("parser: VAR_GLOBAL standalone (GVL file)", () => {
	it("GVL file", () => {
		...
		`);
		expect(errors).toEqual([]);
		const gvl = units[0] as import("./ast.js").GlobalVarList;
...
END_NAMESPACE`;
		...
		expect(errors).toEqual([]);
		expect(units).toHaveLength(1);
		const ns = units[0] as import("./ast.js").Namespace;
...
END_FUNCTION_BLOCK`;
		...
		const ie = decl?.type as import("./ast.js").ImplicitEnumType;
...
END_FUNCTION_BLOCK`;
		...
		expect(errors).toEqual([]);
		const fb = units[0] as FunctionBlock;
		const ie = fb.varSections[0]?.decls[0]?.type as import("./ast.js").ImplicitEnumType;
