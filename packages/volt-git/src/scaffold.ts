...
function readme(plcProjectName: string): string {
	return [
		...
		"",
		"`src/` mirrors the IDE — edit `.st` files locally (every writable source is `.st`; FBD/LD graphical",
		"bodies are `.st` too, editable as VG text); `volt push` writes them back. `.cfc`/`.sfc` are read-only",
