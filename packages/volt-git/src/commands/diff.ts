...
import { outgoingDiffs, type FileDiff } from "../git.js";
import { RANGE } from "../domain/ide-tree.js";

type DiffResult = { kind: "ok"; diffs: FileDiff[] } | { kind: "error"; reason: string };

export function diff(workspace: string): DiffResult {
	const root = resolve(workspace);
	try {
		return { kind: "ok", diffs: outgoingDiffs(root, RANGE, "src") };
	} catch (err) {
