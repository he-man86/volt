...
import { configExists, saveConfig, type WorkspaceConfig } from "../config.js";
import { gitInit, isInsideRepo, commitAll, headCommit, currentBranch, readTreeToIndex, resolveGitDir, updateRef } from "../git.js";
import { materializeItem } from "../domain/materialize.js";
import { writeSrcFiles, ensureGitignore } from "../files.js";
import { writeWorkspaceScaffold } from "../scaffold.js";
import { buildVoltIdeTree, commitVoltIde, RANGE } from "../domain/ide-tree.js";
import { saveIdeRefs } from "../domain/sidecar.js";
...
export async function init(workspace: string, bridge: Remote, opts: { onProgress?: ProgressHandler } = {}): Promise<InitResult> {
	...
	// Init seeds the whole IDE (no prior volt/ide tree), so `ideFiles` is everything and there is no parent.
	const tree = buildVoltIdeTree(gitDir, head ?? undefined, undefined, ideFiles, []);
