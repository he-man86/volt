...
import { materializeItem } from "../domain/materialize.js";
import { ensureGitignore, stripSrcPrefix, writeSrcFiles } from "../files.js";
import { buildVoltIdeTree, commitVoltIde, RANGE, voltIdeHead } from "../domain/ide-tree.js";
import { loadIdeRefs, saveIdeRefs, type IdeRefs } from "../domain/sidecar.js";
import { buildStatusData, computeIncoming } from "../domain/status-model.js";
...
export async function pull(root: string, bridge: Remote, opts: PullOptions = {}): Promise<PullResult> {
	...
	const synced = [...incoming.added, ...incoming.modified, ...incoming.removed].sort();
	...
	if (sidecar !== undefined && fetched.projectVersion === sidecar.projectVersion && synced.length === 0) {
		return { kind: "ok", synced, message: "already up to date with the IDE", status: postStatus() };
	}
	if (opts.dryRun === true) {
		return { kind: "ok", synced, message: "dry run — these IDE items would be merged in", status: postStatus() };
	}
	...
	return { kind: "ok", synced, status: postStatus() };
}
