/**
 * volt-git show <ref> <src-relative-path> → raw file bytes at a ref. Used by the vscode 3-way merge
 * editor + file-restore. Refs: HEAD / any git ref, the merge sides (MERGE_OURS=HEAD,
...
export async function show(root: string, bridge: Remote, ref: string, rel: string): Promise<Buffer | { error: string }> {
	...
	const gitRef =
		ref === "MERGE_OURS" ? "HEAD" : ref === "MERGE_THEIRS" ? "MERGE_HEAD" : ref === "MERGE_BASE" ? mergeBase(root, "HEAD", "MERGE_HEAD") : ref;
