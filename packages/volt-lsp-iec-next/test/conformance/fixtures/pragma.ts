...
export const PRAGMA_TESTS: readonly LanguageTest[] = [
  ...
  // NOTE: This catalog covers FB-position pragmas. GVL-specific
  // pragma tests (qualified_only, etc.) are tracked in their own
  // catalog (variable-section-tests.ts / pragma-tests sections
  // that exercise GVL-scope pragmas via direct GVL sourceText).
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_warning_message
VAR
	iVar : INT;
END_VAR
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_unknown_attribute_typo
VAR
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_info_message
VAR
	iVar : INT;
END_VAR
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_displaymode_invalid_value
VAR
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_no_init_aliases
VAR
	{attribute 'noinit'}
	iA : INT;
	{attribute 'no_init'}
...
`,
  ...
  // the first mixes VAR_INPUT + VAR in a child decl that splits
  // awkwardly; the second is a property with only GET (no SET),
  // which the splitter's accessor pairing logic assumes. Add when
  // the splitter is extended.
...
`,
...

  {
    ...
    source: `FUNCTION_BLOCK FB_LANG_error_message
VAR
	iVar : INT;
END_VAR
...
`,
  ...
  {
    name: "region_pragma_basic",
    pouName: "FB_LANG_region_pragma",
    kind: "function_block",
    ...
    source: `FUNCTION_BLOCK FB_LANG_region_pragma
VAR
	iCount : INT;
END_VAR
...
`,
  ...
  {
    name: "region_pragma_nested",
    pouName: "FB_LANG_region_nested",
    kind: "function_block",
...
`,
...

  {
    ...
    plcPrgBody: "fb_pud();",
    source: `{TOTALLY_MADE_UP_DIRECTIVE foo}
...
`,
...

  {
    ...
    plcPrgBody: "fb_pcp();",
    source: `{attribute 'pin_presentation_order_inputs' := 'iA, iB'}
