...
const RECORDINGS: ReadonlyArray<{ vendor: Vendor; filename: string; floor: number }> = [
  ...
  // + oop/ (interface-implementation) + pragmas/ (message + orphan-conditional): 230 TC / 227 CS of 259.
  // Remaining non-agreements are documented IDE-only divergences (parse cascades, app-config warnings,
  // op_sys_* / __-system constructs) — not reproducible offline; the subset (no-FP) gate stays green on them.
  { vendor: "twincat", filename: "expected-tc.json", floor: 230 },
  { vendor: "codesys", filename: "expected-codesys.json", floor: 227 },
]
