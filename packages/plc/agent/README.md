...
├── .plcassist-st-lsp-version   # records which corpus version is installed
└── .plcassist/                 # PLC Assist's internal state — invisible to your editor
...
`.plcassist/` is ours. Your own `.git/` (if you `git init` the workspace yourself) is yours. They don't touch each other.
...
Some operations make sense for a human at a terminal but are risky for an AI to invoke autonomously — `force` is the canonical example. The MCP `plc_push` tool exposes a `force` parameter to the AI, but it's **gated on a filesystem capability lease** the human grants via the CLI:

```bash
$ plc grant push-force --ttl 5m --once
granted: push-force for 5m (one-shot)
expires: 2026-05-25T20:32:00.000Z (4m 59s remaining)
lease:   .plcassist/auth/push-force.lease
```

The AI then sees `availableCapabilities: [{ capability: "push-force", oneShot: true, ... }]` in its next `plc_status` response and can call `plc_push({ force: true })`. On success with a one-shot lease, the lease is consumed; the next force attempt fails until the human grants again. Without an active lease, AI's `force: true` is rejected with `status: "force_unauthorized"` and the exact CLI command the human must run.
...
│   ├── config.ts              .plcassist/config.json — workspace binding
│   ├── snapshot.ts            .plcassist/snapshot/ — hidden bare repo for diff;
│   │                          ChangeSet type + computeIncoming/Outgoing
│   ├── lease.ts               .plcassist/auth/ — capability leases (sudo-style gate
...
│   ├── plc_init.ts            registerPlcInit
│   ├── plc_pull.ts            registerPlcPull (exposes dryRun)
│   ├── plc_push.ts            registerPlcPush (exposes force + dryRun;
│   │                          force gated on push-force lease)
│   ├── plc_status.ts          registerPlcStatus (surfaces availableCapabilities)
│   ├── plc_compile.ts         registerPlcCompile
│   ├── bin.ts                 `plc-mcp` executable — buildServer + stdio transport
...
`plc-mcp` is the same verb surface as the CLI, exposed as an MCP server over stdio. Any MCP client (Claude Desktop, opencode, Cursor, custom) can drive the workspace.

Tool names mirror the CLI: `plc_init`, `plc_pull`, `plc_push`, `plc_status`, `plc_compile`. Inputs mirror the flags. Outputs are structured JSON so the AI can reason about `drift_detected` / `rejected` / `nothing_to_push` / `force_unauthorized` / `would_push` (dry-run) without parsing prose.

`plc_status` returns `incoming` and `outgoing` ChangeSets (same shape as `hg incoming` / `hg outgoing`) plus `availableCapabilities` so the AI can see which elevated parameters the human has granted, without trial-and-error.
...
```json
{
  "mcpServers": {
    "plc": {
      "command": "node",
      "args": ["/abs/path/to/packages/plcassist-agent/dist/tools/bin.js"],
      "env": {
        "PLCASSIST_WORKSPACE": "/abs/path/to/your/workspace",
        "PLCASSIST_BRIDGE_PORT": "8555"
      }
    }
  }
}
...
`PLCASSIST_WORKSPACE` is optional — tools accept a `workspace` arg per call too, in case you want one MCP server to serve multiple projects. Same for `PLCASSIST_BRIDGE_PORT` / `port`.

Flags:

| Flag | Default | Applies to | Purpose |
|---|---|---|---|
| `--port N` | `8555` (env `PLCASSIST_BRIDGE_PORT`) | all | bridge port |
...
PLCASSIST_BRIDGE_PORT=8555 npm run conformance         # CLI / wire conformance
