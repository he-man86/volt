 ...
 * `<workspace>/.plcassist/config.json`.
 *
 ...
 * detection beyond looking up the `.plcassist/` directory.
 */
...
export interface WorkspacePaths {
	...
	root: string;
	/** Absolute path to the hidden `.plcassist/` directory. */
	stateDir: string;
	 ...
	 * (`.plcassist/auth/`). Each granted capability is one
...
export function workspacePaths(workspaceRoot: string): WorkspacePaths {
	...
	const stateDir = join(root, ".plcassist");
	return {
		root,
		stateDir,
...
export function loadConfig(workspaceRoot: string): WorkspaceConfig {
	...
	if (!existsSync(configPath)) {
		throw new Error(
			`no plcassist workspace at ${root} (missing .plcassist/config.json) — run \`plc init\` first`,
		);
	}
