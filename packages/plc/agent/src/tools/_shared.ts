...
const DEFAULT_BRIDGE_PORT = Number.parseInt(
	process.env.PLCASSIST_BRIDGE_PORT ?? "8555",
	10,
);
const DEFAULT_WORKSPACE = process.env.PLCASSIST_WORKSPACE ?? process.cwd();
...
export const commonArgs = {
	workspace: z
		...
		.optional()
		.describe(
			"Absolute or relative path to the workspace folder. Defaults to env PLCASSIST_WORKSPACE then process cwd.",
		),
	port: z
		.number()
		.int()
		.positive()
		.optional()
		.describe("Bridge HTTP port. Defaults to env PLCASSIST_BRIDGE_PORT then 8555."),
};
