...
**LSP lexer status:**
- `packages/plcassist-st-lsp/src/lexer/tokens.ts:172` (`ALL_KEYWORDS`) currently enumerates ~85 keywords. **It will need to be cross-checked against this corpus** during Stage 1 — likely missing some of the system operators (`__POSITION`, `__CURRENTTASK`, `__POUNAME`, etc.) and the more exotic data type aliases (`LDT`, `LTOD`, `LDATE_AND_TIME`).
