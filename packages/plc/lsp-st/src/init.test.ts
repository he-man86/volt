/**
 * Tests for `plcassist-st-lsp init`. Runs in a temp dir per test so
...
async function makeFixture(): Promise<{ tmpDir: string; sourceDir: string }> {
	const tmpDir = await mkdtemp(join(tmpdir(), "plcassist-init-"));
...
describe("init: error cases", () => {
	it("throws when source corpus is missing", async () => {
		const tmpDir = await mkdtemp(join(tmpdir(), "plcassist-init-err-"));
		await expect(
			runInit({
				targetDir: tmpDir,
