 ...
 *
 * Called from:
 *   - `plc init` (plcassist-agent) — to write the detected vendor into
 *     `.plcassist/config.json`
 *   - VS Code extension — to resolve `plcassist.structuredText.vendor: "auto"`
 */
...
async function walk(
	...
	for (const entry of entries) {
		...
		if (entry === "node_modules" || entry === ".git" || entry === ".plcassist" || entry === "dist") continue;
