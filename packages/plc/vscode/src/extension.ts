...
interface PlcLanguage {
	...
	displayName: string;
	configKey: string;
	/** Workspace settings root for this language (e.g. "plcassist.structuredText"). */
	settingsRoot: string;
...
async function startLanguageClient(
	...
	statusItem.tooltip = "PLC Assist language server status";
	statusItem.command = `plcassist.${lang.languageId}.showOutput`;
	...
	try {
		...
		statusItem.tooltip = `PLC Assist (${lang.displayName}) — running`;
	} catch (err) {
		statusItem.text = `$(error) ${lang.displayName}`;
		statusItem.tooltip = `PLC Assist (${lang.displayName}) — failed to start`;
...
function registerCommands(context: vscode.ExtensionContext): void {
	context.subscriptions.push(
		vscode.commands.registerCommand("plcassist.restart", async () => {
			for (const lang of PLC_LANGUAGES) {
				await restartClient(context, lang);
			}
			vscode.window.setStatusBarMessage("PLC Assist: language server(s) restarted", 2000);
		}),
	);
	...
	for (const lang of PLC_LANGUAGES) {
		context.subscriptions.push(
			vscode.commands.registerCommand(
				`plcassist.${lang.languageId}.showOutput`,
				() => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("plcassist.openReference", async () => {
			const folder = vscode.workspace.workspaceFolders?.[0];
			if (folder === undefined) {
				vscode.window.showWarningMessage(
					"PLC Assist: no workspace folder open. Open a project first.",
				);
				return;
			}
