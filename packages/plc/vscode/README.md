...
2. **Language server** — spawned by the extension when a `.st` file is opened. Delivers hover (with CODESYS docs inline), goto-definition, references, document/workspace symbols, call hierarchy, type hierarchy, completion + signature help, 11 diagnostic checks, semantic tokens, folding ranges, document highlight, selection ranges, and code actions. The server lives in `@plcassist/st-lsp`; the extension finds it via Node module resolution from the workspace.
...
code --extensionDevelopmentPath="<repo root>/packages/plcassist-vscode" .
```

Open any `.st` file in the new VS Code window. Syntax highlighting is immediate; the LSP starts on first `.st` activation (look for "PLC Assist — Structured Text" in the Output panel).

## Install (packaged `.vsix`)

```bash
npm run --workspace=@plcassist/vscode package
# produces plcassist-vscode-0.0.1.vsix in packages/plcassist-vscode/
code --install-extension packages/plcassist-vscode/plcassist-vscode-0.0.1.vsix
```

## Resolving the LSP server

The extension auto-discovers `@plcassist/st-lsp` by walking up from your workspace folder looking for `node_modules/@plcassist/st-lsp/dist/bin.js`. If your workspace doesn't have the package as a dep, the extension also checks the extension's install dir and the global npm dir.

Explicit override (settings):

```json
{
  "plcassist.structuredText.lspServer": "C:/path/to/plcassist-st-lsp/dist/bin.js"
}
```
...
All of the LSP's tunables are exposed as VS Code settings under `plcassist.structuredText.*`. Changes hot-reload — the language server restarts when a setting changes.
...
- `PLC Assist: Restart Language Server` — restart all PLC LSP clients
- `PLC Assist: Show Structured Text Output` — open the LSP output channel
- `PLC Assist: Open CODESYS Language Reference` — open the local corpus (offers to run `plc init` if missing)
...
- ✅ Hover with CODESYS docs (via `@plcassist/st-lsp`)
...
## Tip: PLC Assist's `(* folder: X *)` annotation
