/**
 * Per-workspace capability leases — the "sudo" of volt-agent.
 *
 ...
 * surfaced in `volt_status` so the AI can see which capabilities are
 ...
 *   - `.volt/` is gitignored, so leases never leak into a
 *     committed repo.
 */
import {
	existsSync,
 ...
 * is deleted) so subsequent reads are fast and so `volt_status`
 ...
 * by `volt_status` to surface available capabilities to the AI without
