...
function plcAt(port: number, workspace: string, ...args: string[]): CliResult {
	const r = spawnSync(
		...
		{ encoding: "utf-8", env: { ...process.env, VOLT_BRIDGE_PORT: String(port) } },
	);
...
async function runAllScenarios(rootTmp: string): Promise<void> {
	...
	section("N06 — plc init on a workspace bound to a different project");
	{
		...
		mkdirSync(join(ws, ".volt"), { recursive: true });
		writeFileSync(
			join(ws, ".volt", "config.json"),
			JSON.stringify({
				schemaVersion: 1,
	...
	section("N09 — .gitignore: created if absent, appended if present, preserves user entries");
	{
		...
		assert(/\.volt/.test(contentA), "case A: .gitignore contains the .volt/ entry");
		...
		assert(/\/?\.volt\/?/m.test(contentB), "case B: our .volt/ entry was appended");
