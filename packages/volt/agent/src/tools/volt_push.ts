/**
 * `volt_push` MCP tool — push workspace state to the IDE.
 *
 ...
 * list (`incoming`) so the human can decide whether to volt_pull or
 * grant a force lease.
 */
import { z } from "zod";
...
export function registerVoltPush(server: McpServer): void {
	server.registerTool(
		"volt_push",
		{
			description:
					...
					return jsonContent({
						...
						...(result.dryRun === true && {
							...
							hint:
								...
								"Call volt_push without dryRun to actually send.",
						}),
					});
				}
