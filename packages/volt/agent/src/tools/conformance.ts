#!/usr/bin/env node
/**
 * MCP e2e — drive the `volt-mcp` server over stdio with a real MCP
 ...
 *   M02 — volt_status on an uninitialized workspace
 *   M03 — volt_init
 *   M04 — volt_pull (first pull from IDE)
 *   M05 — volt_status post-import (clean)
 *   M06 — volt_push with a single edit
 *   M07 — volt_compile
 *
...
// `volt-mcp` bin lives alongside this script at dist/tools/bin.js.
...
async function main(): Promise<void> {
	...
	const rootTmp = mkdtempSync(join(tmpdir(), "volt-mcp-e2e-"));
	...
	const transport = new StdioClientTransport({
		...
		env: {
			...process.env,
			VOLT_BRIDGE_PORT: String(BRIDGE_PORT),
			VOLT_WORKSPACE: workspace,
		},
	});
	const client = new Client({ name: "volt-mcp-e2e", version: "0.0.0" }, { capabilities: {} });
...
async function runScenarios(client: Client, workspace: string): Promise<void> {
	...
	section("M01 — listTools advertises the 5 expected tools");
	{
		...
		const expected = ["volt_compile", "volt_init", "volt_pull", "volt_push", "volt_status"];
		assert(
			JSON.stringify(names) === JSON.stringify(expected),
			"tools = [volt_compile, volt_init, volt_pull, volt_push, volt_status]",
	...
	// ─── M02: volt_status on uninitialized workspace ──────────────────
	section("M02 — volt_status on an uninitialized workspace");
	{
		const r = await client.callTool({ name: "volt_status", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_status didn't error pre-init", error);
	...
	// ─── M03: volt_init ───────────────────────────────────────────────
	section("M03 — volt_init binds the workspace");
	{
		const r = await client.callTool({ name: "volt_init", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_init succeeded", error);
		assert(data?.status === "initialized", `status=initialized, got ${String(data?.status)}`);
		assert(existsSync(join(workspace, ".volt", "config.json")), ".volt/config.json exists");
	}

	// ─── M04: volt_pull populates workspace ─────────────────────────
	section("M04 — volt_pull populates the workspace");
	{
		const r = await client.callTool({ name: "volt_pull", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_pull succeeded", error);
	...
	// ─── M05: volt_status post-import is clean ────────────────────────
	section("M05 — volt_status post-import: IDE in sync, workspace clean");
	{
		const r = await client.callTool({ name: "volt_status", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_status succeeded", error);
	...
	// ─── M06: volt_push with an edit ────────────────────────────────
	section("M06 — volt_push with a single edit");
	{
		...
		const r = await client.callTool({ name: "volt_push", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_push succeeded", error);
	...
	section("M07 — volt_push & volt_pull dry-run: AI can preview without mutating");
	{
		// 7a. dry-run pull on a clean baseline — nothing should change.
		// First make sure we're up to date.
		const baseline = await client.callTool({ name: "volt_pull", arguments: {} });
		...
		// 7b. volt_pull dryRun: must REPORT the incoming addition WITHOUT
		...
		await client.callTool({ name: "volt_pull", arguments: {} });
	...
	section("M08 — volt_push force is GATED on a `push-force` lease");
	{
		...
		const preGrant = await client.callTool({ name: "volt_status", arguments: {} });
		...
		assert(
			...
			"volt_status reports empty availableCapabilities pre-grant",
		...
		const postGrant = await client.callTool({ name: "volt_status", arguments: {} });
		...
		assert(
			...
			"volt_status surfaces the granted push-force lease (one-shot)",
	...
	// ─── M10: volt_compile ────────────────────────────────────────────
	section("M10 — volt_compile returns a structured diagnostics report");
	{
		const r = await client.callTool({ name: "volt_compile", arguments: {} });
		const { data, error } = parseToolJson(r as ToolJson);
		assert(error === undefined, "volt_compile succeeded", error);
		assert(
