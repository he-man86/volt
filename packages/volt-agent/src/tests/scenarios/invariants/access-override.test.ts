 ...
 *   - `"off"` â€” skip the extension entirely on pull; no file lands.
 *   - `"rw"` â€” flip a default-`r` extension to writable; push goes
...
describe("scenario: extensionAccess overrides", () => {
	test('"off" makes a library extension invisible to the workspace', async () => {
		...
		// Source POUs still land â€” only `.library` got filtered.
	...
	test('"rw" override lets push send a normally-read-only extension', async () => {
		...
		// Either accepted (bridge consumes the edit) or â€” depending on
		// TestBridge's policy for non-source kinds â€” refused at the
