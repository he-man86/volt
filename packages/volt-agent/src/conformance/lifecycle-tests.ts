...
`,
	...
	// syntax in PLC_PRG's VAR section, and volt-agent's push pipeline
	// parser (st-parse.ts) doesn't accept the `(arg := val)` shortcut.
	// TC supports it. This is a real LSP parser gap worth re-adding
	// once the parser is extended. Tracked separately.
...
`,
...

	{
		...
		kind: "function_block",
		feature: "FB_Init missing the bInCopyCode parameter — should warn/error",
		fromDoc: "11-fb-lifecycle.md#fb_init",
		expectTcAccepts: true,
		note: "Docs say spec is fuzzy on whether TC errors. LSP fbLifecycleSignature should catch the missing canonical param either way.",
...
`,
...

	{
		...
		kind: "function_block",
		feature: "FB_Exit missing the bInCopyCode parameter — should error per docs",
		fromDoc: "11-fb-lifecycle.md#fb_exit",
		expectTcAccepts: true,
		note: "Docs say FB_Exit missing bInCopyCode is an error. Test verifies TC behavior + LSP catch.",
