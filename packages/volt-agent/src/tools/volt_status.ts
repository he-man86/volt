/**
 * `volt_status` MCP tool — show what differs between IDE / snapshot / workspace.
 *
 ...
 *                        `volt_pull` would bring INTO the workspace
 *                        (= `hg incoming` / git's `HEAD..@{u}`)
 *   - outgoing         — {added, removed, modified} POU lists that
 *                        `volt_push` would send TO the bridge; same
 ...
 *                          check this BEFORE calling volt_push with force, etc.
 *
 * Cheap and safe to call at any time — including before `volt_init`.
 */
...
export function registerVoltStatus(server: McpServer): void {
	server.registerTool(
		"volt_status",
		{
			description:
				"Report what differs between the IDE, the last-pulled snapshot, and the workspace. Returns `incoming` (what volt_pull would bring into the workspace) and `outgoing` (what volt_push would send to the bridge) as per-item added/removed/modified lists — same shape, same direction terms as `hg incoming`/`hg outgoing`. Plus per-file workspace dirtiness, a next-action recommendation (pull / push / reconcile / init / null), and a human-readable summary. Cheap — run as often as needed to orient before pull/push.",
