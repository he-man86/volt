/**
 * `volt_pull` MCP tool — pull IDE state into the workspace.
...
export function registerVoltPull(server: McpServer): void {
	server.registerTool(
		"volt_pull",
		{
			description:
		...
		async (args) => {
			...
			return jsonContent({
				...
				hint: dryRun
					? upToDate
						? "Dry-run: workspace already matches the IDE — a real volt_pull would be a no-op."
						: "Dry-run: shows what volt_pull WOULD bring in (see `incoming`). Snapshot and workspace were NOT touched."
					: upToDate
					? "Workspace already matches the IDE. Call volt_status to see what (if anything) you've changed locally."
					: `${v.written.length} file(s) written, ${v.removed.length} removed. Edit files in the workspace, then call volt_push to send back.`,
			});
		},
	);
}
