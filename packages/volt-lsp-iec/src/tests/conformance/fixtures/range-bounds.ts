...
export const RANGE_BOUNDS_TESTS: readonly LanguageTest[] = [
	// ─── Subrange: constant initializer vs declared bounds ──────────────
	{
		...
		plcPrgBody: "fb_sir();",
		source:
`FUNCTION_BLOCK FB_LANG_subrange_init_in_range
VAR
	value : INT(1..100) := 50;
END_VAR
...
`,
	},
	{
		...
		plcPrgBody: "fb_sar();",
		source:
`FUNCTION_BLOCK FB_LANG_subrange_init_above_range
VAR
	value : INT(1..100) := 200;
END_VAR
...
`,
	},
	{
		...
		plcPrgBody: "fb_sbr();",
		source:
`FUNCTION_BLOCK FB_LANG_subrange_init_below_range
VAR
	value : INT(-10..10) := -20;
END_VAR
...
`,
	...
	// ─── Subrange: constant assignment in a method body ──────────────────
	{
		...
		source:
`FUNCTION_BLOCK FB_LANG_subrange_assign_const_out
VAR
	value : INT(1..100);
END_VAR
...
`,
	...
	// ─── Array: constant index vs declared bounds ────────────────────────
	{
		...
		source:
`FUNCTION_BLOCK FB_LANG_array_index_const_in_bounds
VAR
...
`,
	},
	{
		...
		source:
`FUNCTION_BLOCK FB_LANG_array_index_const_out_of_bounds
VAR
	arr : ARRAY[1..10] OF INT;
END_VAR
...
`,
	},
	{
		...
		source:
`FUNCTION_BLOCK FB_LANG_array_index_const_below_bounds
VAR
	arr : ARRAY[1..10] OF INT;
END_VAR
