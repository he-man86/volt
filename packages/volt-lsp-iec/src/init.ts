 ...
 *     ├─ .volt-lsp-iec-version           ← version marker
...
// Dev/npm layout: the docs tree lives in the package (PKG_DIR/docs). In the compiled `volt`/`volt-lsp-iec`
...
const VERSION_MARKER_REL_PATH = `${SKILL_DIR_REL}/.volt-lsp-iec-version`;
...
Authoritative CODESYS Structured Text language reference, installed into this workspace by \`volt-lsp-iec init\` (run via \`volt init\`). Use when writing or reviewing \`.st\` files.
	...
	try {
		...
		throw new Error(
			`Source corpus not found at ${sourceCodesysDir}. ` +
				`The volt-lsp-iec package may be missing the docs/ tree.`,
		);
	}
