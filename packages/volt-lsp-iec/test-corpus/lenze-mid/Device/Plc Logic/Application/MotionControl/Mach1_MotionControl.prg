...
END_NETWORK
NETWORK 4 LD
  xWrappingDeviceDoneOnce := L_MC1P_ChangeMachineData_WrappingDevice(Axis := Axis_OverrollingDevice, xExecute := (Axis_OverrollingDevice.xCommunicationOK AND NOT Axis_OverrollingDevice.xDriveEnabled), xSetFeedconstant := , lrFeedconstant := , xSetGearFactor := , dwGearDenominator := , dwGearNumerator := , xSetAddGearFactor := , dwAddGearDenominator := , dwAddGearNumerator := , xSetPosResolution := , dwPosResolution := , xSetOrientation := TRUE, xOrientation := NOT LST_InputsOutputs.I100_5_ConfigSelectionRightMachine, xSetTraversingRange := , eTraversingRange := , xSetCycleLength := , lrCycleLength := ) SET;
END_NETWORK
NETWORK 5 LD
