...
NETWORK 0 LD TITLE: "Network 1: Averaging of sensorsignal"
  ...
  LET en1 := ;
  IF en1 THEN fc_MeanValue(20, True, i1, db_MeanValuesSideCorrectionSensor.MeasurementValues); END_IF
END_NETWORK
NETWORK 1 LD
  LET g62 := TRUE;
  LET en1 := (g62 AND DB_Kantcorrectie.CopyMeasuredToDark);
  IF en1 THEN DB_Kantcorrectie.CopyMeasuredToDark R= MOVE(DB_Kantcorrectie.MeasuredValue); END_IF
  LET en2 := (g62 AND DB_Kantcorrectie.CopyMeasuredToLight);
  IF en2 THEN DB_Kantcorrectie.CopyMeasuredToLight R= MOVE(DB_Kantcorrectie.MeasuredValue); END_IF
END_NETWORK
NETWORK 2 LD TITLE: "Network 2: Read out inputvalue and calculate correctionvalue"
  LET i1 := REAL_TO_INT(tReal);
  LET en1 := SidecorrectionCalculation(DB_Kantcorrectie.MeasuredValue, DB_Kantcorrectie.Setpoint, -1, DB_Kantcorrectie.LightValue, DB_Kantcorrectie.DarkValue, DB_Kantcorrectie.NoWrapperPercentage);
  IF en1 THEN MOVE(i1); END_IF
END_NETWORK
NETWORK 3 LD TITLE: "Network 3: Determine side-correction value"
  ...
  IF en1 THEN MOVE(DB_Kantcorrectie.Percentage10); END_IF
...
NETWORK 4 LD TITLE: "DONE Network 4: In case a limit switch is reached: overwrite values"
  ...
  IF en1 THEN MOVE(-100.0); END_IF
  LET en2 := MOVE(100.0);
  IF en2 THEN MOVE(-1); END_IF
  LET en3 := MOVE(0.0);
  IF en3 THEN MOVE(1); END_IF
END_NETWORK
