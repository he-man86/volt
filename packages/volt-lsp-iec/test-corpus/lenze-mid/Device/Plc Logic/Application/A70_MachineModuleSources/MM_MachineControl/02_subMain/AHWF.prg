...
NETWORK 5 LD TITLE: "TODO Network 4:"
  ...
  LET en1 := MOVE(750);
  IF en1 THEN Data_Exchange_Motion.FeedForwardWrapper.Control.TorqueLimit := MOVE(100); END_IF
  ...
  LET en2 := g330;
  IF en2 THEN Data_Exchange_Motion.FeedForwardWrapper.Control.VelUnit := MOVE(500); END_IF
  LET i1 := DINT_TO_LREAL(Mach1_Data.Settings.Dints.WrapperStopPosition)/10;
  LET en3 := g328;
  IF en3 THEN Data_Exchange_Motion.FeedForwardWrapper.Control.PosUnit := MOVE(i1); END_IF
