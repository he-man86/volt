...
NETWORK 0 LD "Network 1: Detection of wrapper (set and reset)"
  LET g10 := True;
  Mach1_AuxData.MemWrapperPassedUnderTheSensor := (g10 AND LST_InputsOutputs.I101_0_Wrapper_present) SET;
  LET g1 := (g10 AND NOT LST_InputsOutputs.I101_0_Wrapper_present AND Mach1_AuxData.MemWrapperPassedUnderTheSensor) SET;
  Mach1_AuxData.MemWrapperPresentForLeafCarrier := g1;
  Mach1_AuxData.MemWrapperDetected_ResetLimitedSpeed := g1;
  LET g2 := fc_CamC_CP_UDT(MOVE(g10, Mach1_Data.CamControls.TakeOverCycle_C.Stop), HMI_Var.Mach1.Position, Mach1.GenFlags.Rotflag, Mach1_AuxData.CamControls.TakeOverCycleStopPulse_CP) SET;
  Mach1_AuxData.MemWrapperPresentForLeafCarrier := g2;
  Mach1_AuxData.MemWrapperPassedUnderTheSensor := g2;
END_NETWORK
NETWORK 1 LD
  LET g1 := R_TrigStartPos(CLK := HMI_Var.HMI_AHWF_StartPos) SET;
  Mach1_AuxData.MemWrapperPresentForLeafCarrier := g1;
  Mach1_AuxData.MemWrapperPassedUnderTheSensor := g1;
END_NETWORK
NETWORK 2 LD "Network 2: FB for DTA"
...
NETWORK 5 LD "TODO Network 4:"
  ...
  HMI_Var.HMI_AHWF_fast_winding := Mach1_AuxData.Edge.OSP_StopFlag(CLK := (g328 AND (NOT Mach1_MIDS.FB_Mach1.iStop1 OR NOT Mach1_MIDS.FB_Mach1.iStop2 OR NOT Mach1_MIDS.FB_Mach1.iStop3 OR NOT Mach1_Safety.Status.AllDoorsActuallyClosed OR LST_InputsOutputs.I101_0_Wrapper_present))) SET;
END_NETWORK
NETWORK 6 LD "Network 5: Side correction"
  SideCorrection();
END_NETWORK
NETWORK 7 LD "Network 6: DTA-alarm: side limit reached"
  Mach1.GenFlags.StopDriveDirect := (Alarms_V5_1_100(Mach1_Alarms, (Mach1_AuxData.Edge.OSP_LimitDTA_Reached(CLK := (Mach1_AuxData.MIDS_Active AND Mach1.GenFlags.DelayAfterEmergStop AND (NOT Mach1_Data.Drives.SideCorrection_FreeLimit.Status.Limit_Left_OK OR NOT Mach1_Data.Drives.SideCorrection_FreeLimit.Status.Limit_Right_OK))) AND Mach1_AuxData.AllDrivesHomed), Mach1.GenFlags.StartFlag, Mach1_Alarms.Alm057, Mach1.GenFlags.MinorAlarm) OR (LST_General.AlwaysOff AND Alarms_V5_1_100(Mach1_Alarms, (Mach1_AuxData.MIDS_Active AND Mach1.GenFlags.DelayAfterEmergStop AND (NOT Mach1_Data.Drives.SideCorrection_FreeLimit.Status.Limit_Left_OK OR NOT Mach1_Data.Drives.SideCorrection_FreeLimit.Status.Limit_Right_OK) AND Mach1_AuxData.AllDrivesHomed), Mach1.GenFlags.StartFlag, Mach1_Alarms.Alm074, Mach1.GenFlags.Warning))) SET;
END_NETWORK
NETWORK 8 LD "Network 7: Alarm: Water receptacle full"
  Mach1.GenFlags.StopDriveDirect := Alarms_V5_1_100(Mach1_Alarms, (Mach1_AuxData.IEC_TIMERS.WaterReceptacleFull(IN := (Mach1_AuxData.MIDS_Active AND LST_InputsOutputs.I101_3_Water_level_control AND NOT Mach1_Alarms.Alm055), PT := T#5M) AND HMI_Var.Test_Prod AND HMI_Var.Btn_Spraying AND Mach1_AuxData.AllDrivesHomed), Mach1.GenFlags.StartFlag, Mach1_Alarms.Alm055, Mach1.GenFlags.MinorAlarm) SET;
END_NETWORK
