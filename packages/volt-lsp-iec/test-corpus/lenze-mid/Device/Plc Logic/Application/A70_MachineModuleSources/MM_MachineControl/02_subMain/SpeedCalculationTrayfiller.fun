...
NETWORK 0 LD TITLE: "NETWORK 1: Write setpoint speed in DB"
  ...
  LET en1 := ;
  IF en1 THEN tInt := (Mach1_Data.Drives.FeedForwardADS.Control.AutoSpeed + 30); END_IF
  LET en2 := en1;
  IF en2 THEN Mach1_Data.Drives.FeedForwardATF.Control.AutoSpeed := MOVE(tInt); END_IF
END_NETWORK
NETWORK 1 LD TITLE: "NETWORK 2: Activation + Runtime guard feed forward dryer/trayfiller"
  ...
  IF en1 THEN (Mach1_Data.Drives.FeedForwardATF.Control.AutoSpeed >= 10); END_IF
  Mach1.GenFlags.StopDriveDirect S= RuntimeGuard_V5_1_100(Mach1_Alarms, (Mach1_AuxData.TrayfillerActive AND Mach1_MIDS.IDB_TrayFiller.oFeedForwardMotor AND Mach1.Genflags.DelayAfterSTO AND (Mach1_AuxData.AllDrivesHomed OR NOT Mach1_AuxData.TrayfillerActive)), LST_General.AlwaysOff, en1, T#10S, Mach1.GenFlags.StartFlag, Mach1_Alarms.Alm051, Mach1.GenFlags.MinorAlarm, Mach1_AuxData.IEC_TIMERS.RuntimeGuardFeedforwardTrayfiller, oForwardGuarded => Mach1_Data.Drives.FeedForwardATF.Control.StartAuto);
END_NETWORK
