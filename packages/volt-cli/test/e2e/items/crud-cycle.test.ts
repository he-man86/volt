 ...
 *   delete → assert {item:gone, project:Δ}     (delete)
 */
...
describe(`lifecycle / CRUD cycle (${BASE})`, () => {
	...
	for (const k of LIFECYCLE_KINDS) {
		it(`${k.key}: create→fetch→fixedpoint→edit→rename→move→delete, versions track correctly`, async () => {
			...
			assertDelta(s0, s1, wire, { item: "new", project: true })
			...
			assertDelta(s1, s2, wire, { item: "same", project: false })

			// 5. EDIT content → content-edit deltas (structure stays — same name)
			await updateItem(wire, k.edit(name))
			const s3 = await snapshot()
			assertDelta(s2, s3, wire, { item: "change", project: true })
			expect(await fetchSource(wire)).toMatch(k.editToken)

			// 6. RENAME → projectVersion changes; the renamed item keeps its content version
			...
			// 7. MOVE → item version changes, because the folder is in the item hash.
			//
			// This step is why `structureVersion` is gone. It hashed the item NAMES alone, so a move — the one
			// structural change Volt performs — left it identical, and this test asserted that as correct. A
			// "did the tree shape change" signal that a move does not move is worse than none.
			...
			assertDelta(s5, s6, newWire, { item: "gone", project: true })
		})
	}
})
