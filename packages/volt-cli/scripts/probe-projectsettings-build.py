...
LOG = os.environ.get("VOLT_PROBE_LOG") or os.path.join(HERE, "projectsettings-build.log")
...
try:
    ...
    # Does a BUILD make the settings appear? The language model is what holds them, and a build is what
    # initialises it.
    log("")
    log("=== after a BUILD ===")
    try:
        ...
        if len(apps) > 0:
            log("  building %r ..." % str(apps[0].get_name()))
            apps[0].build()
            log("  build returned")
        else:
            log("  no Application found")
    except Exception:
        log("  build threw: " + traceback.format_exc().splitlines()[-1])

    provider = static_member("_3S.CoDeSys.Engine.APEnvironment", "LMServiceProvider")
    cfgB = provider.GetType().GetProperty("ConfigurationService").GetValue(provider, None) if provider else None
    wcB = cfgB.GetType().GetProperty("WarningConfiguration").GetValue(cfgB, None) if cfgB else None
    if wcB is not None:
        for g in ("GetDisabledWarningIds", "GetWarningAsErrorIds"):
            try:
                v = wcB.GetType().GetMethod(g).Invoke(wcB, None)
                log("  AFTER BUILD %s() -> %r" % (g, None if v is None else list(v)))
            except Exception:
                log("  AFTER BUILD %s() threw" % g)
