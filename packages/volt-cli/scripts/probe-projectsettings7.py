...
LOG = os.environ.get("VOLT_PROBE_LOG") or os.path.join(HERE, "projectsettings7.log")
...
try:
    ...
    save = None
    for src in [store.GetType()] + list(store.GetType().GetInterfaces()):
        for m in src.GetMethods():
            if m.Name == "Save" and len(m.GetParameters()) == 2 and m.GetParameters()[0].ParameterType.IsEnum:
                save = m; break
        if save is not None: break
    rootEnum = save.GetParameters()[0].ParameterType

    for nm in ("Project", "ProjectDefaults", "UserProject", "MachineProject"):
        try:
            val = System.Enum.Parse(rootEnum, nm)
        except Exception:
            continue
        out = os.path.join(tempfile.gettempdir(), "volt-options-%s.xml" % nm)
        try:
            fs = System.IO.FileStream(out, System.IO.FileMode.Create, System.IO.FileAccess.Write)
            try:
                save.Invoke(store, System.Array[System.Object]([val, fs]))
            finally:
                fs.Close()
            log("saved %s -> %s (%d bytes)" % (nm, out, os.path.getsize(out)))
        except Exception:
            log("%s save threw: %s" % (nm, traceback.format_exc().splitlines()[-1]))
    proj.close()
    log("")
    log("done")
except Exception:
