...
| 623 | `PLCDUTALIAS` | `PlcDut` | `dut` | both | ✅ | ✅ | ✅ | the ONE DUT code. TwinCAT reports EVERY DUT (struct/enum/union/alias) as 623 and creates every DUT with it; CODESYS classifies every `IDUTObject` here and creates with one `create_dut` call. One wire kind `dut`. The IDE derives the subtype from the declaration text on both read and create, and so does Volt's materializer — to pick the FILE extension only. Volt never picks a sub-KIND. |
