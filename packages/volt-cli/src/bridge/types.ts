 ...
 * Vendor-agnostic by design: any bridge (Beckhoff, CODESYS, TIA, …)
...
/**
 * Wire-shape: an item on the wire is one assembled `.st` / `.gvl` /
...
export const FetchedItemSchema = z
	.object({
		 ...
		 * workspace file. For graphical POUs (FBD/LD/SFC/CFC), the bridge
		 * transpiles the body to ST before assembling; the agent never
		 * sees raw PLCopenXML.
		 */
		sourceText: z.string(),
