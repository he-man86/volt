...
namespace Volt.Engine.Text
{
    ...
    /// regex over Structured Text, so it belongs with the other ST text readers. Its caller needs it because network text
    ...
    public static class InstanceTypes
    {
        ...
        /// so the writer can restore the <c>typeName</c> that network text does not carry.</summary>
