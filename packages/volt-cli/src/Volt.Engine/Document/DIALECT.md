...
| **D17** | **CLOSED — LD is supported on every element that can hold a body, MEASURED on both vendors.** It had only ever been exercised live on a PROGRAM (plus one FB, to build-verify), so "LD works everywhere" rested on FBD parity and the offline suite. `test/e2e/graphical/graphical-kinds.test.ts` pushes the same rung into a function_block, program, function, method, action and BOTH property accessors, reads it back, re-pushes it byte-identical and builds it: 7/7 on CODESYS SP21 and 7/7 on TwinCAT, first run on each, no vendor divergence. The members are the half that had no coverage at all — a member ladder rides inside the PARENT's document, so the splice has to place it in the member's own body; the fixtures give the parent a TEXTUAL body so that mistake cannot hide. |
...
| **D20** | **OPEN, and it blocks the CODESYS half of that coverage.** `CodesysTestProject.project` predates SP21; saving it under SP21 (the only way to add an object) re-resolves its referenced libraries and pulls in `AppendErrorString`, a HIDDEN function (`IsHidden="true"` in the System/Analyzation `browsercache`) with no readable return type. `LibSignatureRenderer` throws — correctly, it refuses to invent a return type — and the whole `fetch` dies: 593 items to zero. Volt already skips hidden objects in the PROJECT tree (`CodesysTypeMap` → `IHiddenObject`), but the library-signature path carries no hidden flag, so it cannot make the same call. Fix that and the CODESYS fixture can carry a CFC POU too. |
