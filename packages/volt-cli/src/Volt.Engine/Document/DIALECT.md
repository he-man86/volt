...
| D7 | Does TwinCAT nest a CFC body under `<body>/<addData>` the way CODESYS does? Still unmeasured — no TwinCAT CFC or SFC fixture exists, and one cannot be produced from a test because Volt never CREATES a diagram. It is no longer load-bearing: `Document/BodyElement` searches any depth under `addData` and any direct child, so whatever shape the vendor uses is found and refused. The two outcomes were never symmetric — searching too wide costs nothing, missing the element destroys a body that cannot be rebuilt from text. Kept open as a fact about the vendor, not as a risk. |
...
| **D17** | **CLOSED — LD is supported on every element that can hold a body, MEASURED on both vendors.** It had only ever been exercised live on a PROGRAM (plus one FB, to build-verify), so "LD works everywhere" rested on FBD parity and the offline suite. `test/e2e/graphical/ld-kinds.test.ts` pushes the same rung into a function_block, program, function, method, action and BOTH property accessors, reads it back, re-pushes it byte-identical and builds it: 7/7 on CODESYS SP21 and 7/7 on TwinCAT, first run on each, no vendor divergence. The members are the half that had no coverage at all — a member ladder rides inside the PARENT's document, so the splice has to place it in the member's own body; the fixtures give the parent a TEXTUAL body so that mistake cannot hide. |
