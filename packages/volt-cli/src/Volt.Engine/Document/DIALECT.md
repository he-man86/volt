...
| **D4i** | **A POU-INTERNAL MEMBER FOLDER CANNOT SURVIVE A DOCUMENT IMPORT ON TWINCAT — and that is now the whole of what blocks the single-document write there.** The folder IS in the document: TwinCAT's export writes `<Folder Name="Inner"><Object Name="Helper" .../></Folder>` inside `<ProjectStructure>` (fixture `tc-pou/FB_TcFolderedMember.plcopen.xml`), and it is the ONLY place the member's placement is recorded. Its importer ignores it. Measured as a 2x2 — folder pre-created via the scripting API or not, member nested in `<Folder>` in the document or not — and all four cells land the member at the POU ROOT with the folder deleted. (An earlier reading of this said the nested member was DROPPED entirely; that was a probe artifact — `WalkItems` does not descend into a POU, so no member was ever visible to it. The member IS created; only its placement is lost.) And it cannot be put back: `ExportChild` REFUSES a member — *"The tree item 'Deep' cannot be exported seperately because it has no document file"* — so the D4f archive move, which relocates any TOP-LEVEL item, has no member equivalent. `PushService.RestoreChildFolders` therefore fails loudly on this vendor rather than silently flattening, which is correct and is also why the one-document write stays off here. **`ProjectStructure` deliberately writes members FLAT** as a result: the nesting is honoured by neither vendor (CODESYS discards the whole block) and buys nothing |
