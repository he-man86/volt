...
```bash
npm install --save-dev @plcassist/st-lsp
```

The package ships a `plcassist-st-lsp` binary that speaks LSP over stdio.

## Using with VS Code

This package is consumed by the `@plcassist/vscode` extension, which spawns it as a language server. No standalone configuration needed — install the extension and `.st` files light up.
...
```jsonc
{
  "lsp": {
    "plcassist-st": {
      "command": ["plcassist-st-lsp", "--stdio"],
      "extensions": [".st"]
    }
  }
}
...
The package exports an `installCorpus()` helper consumed by `plcassist-agent`'s `plc init` command. When a user runs `plc init` in a PLC workspace, the corpus + a CLAUDE.md pointer is copied into that workspace so any AI session there can read the language reference proactively.

```ts
import { installCorpus } from "@plcassist/st-lsp";
