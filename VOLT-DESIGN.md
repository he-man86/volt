...
| **2 ✅** | `VoltSidePanel` — Volt's own session column (sibling of the git/review + file-explorer panels), mounted via one line in `session.tsx`. Native v2 components (`SegmentedControlV2`/`ButtonV2`), `ResizeHandle`, design tokens. All UI in `volt-app`. | ⚠ `packages/app` ×2 (session.tsx + dep) | 2 | — | ✅ app builds + bundles it; `bun run dev:desktop` to view |
